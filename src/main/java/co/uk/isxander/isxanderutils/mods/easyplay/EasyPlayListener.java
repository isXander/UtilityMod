...
public class EasyPlayListener {
            ...
            if (rawText.toLowerCase().contains("something went wrong trying to send you to that server! if this keeps happening please report it! (")) {
                if (Config.shouldRequeueIfFail()) {
                    if (Config.shouldReplaceErrors()) {
                        ...
                        Messages.sendMessage("§cCouldn't send you to server. Re-queueing.");
                    }
                    ...
                    Messages.sendMessage(Reference.getPrefix(), "§cCouldn't send you to server.");
    ...
    public static void requeuePlayer(String gamemode) {
        Messages.sendMessage(Reference.getPrefix(), "DEVLOG: Attempting to requeue player out of multithread.");
        Multithreading.schedule(() ->
        {
            Messages.sendMessage(Reference.getPrefix(), "DEVLOG: Attempting to requeue player in multithread.");
            Minecraft.getMinecraft().thePlayer.sendChatMessage("/play " + gamemode);
            previousMode = gamemode;
            if (!Config.shouldHideSendMsgs()) {
                return;
            }
            Messages.sendMessage(Reference.getPrefix(), "§aSending you to " + gamemode);
            ...
            Messages.sendMessage(Reference.getPrefix(), "DEVLOG: Finished in multithread.");
        }, Config.getRequeueDelay(), TimeUnit.SECONDS);
        Messages.sendMessage(Reference.getPrefix(), "DEVLOG: Finished out of multithread.");
